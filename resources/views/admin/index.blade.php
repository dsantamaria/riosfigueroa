...
@section('content')
	<div class="col-md-12 messages-resend">
        <h2 class="page-title">Listado de Administradores</h2>
    </div>
    <div id="messages" class="col-xs-12" style="display: none"></div>
    <div class="col-md-8 col-md-offset-2 col-xs-12">
        <div class="panel panel-primary">
            <div class="panel-heading">Listado de Administradores</div>
                                ...
                                <tbody>
                                    @if($users)
                                        @foreach($users as $user)
                                            @if($user->roles[0]->permissions == 'admin')
                                                <tr role="row">
                                                    <td>{{ $user->name }}</td>
                                                    ...
                                                    <td class="table-td-actions">
                                                        @if($user->active == 1) <div class="actions-btn active-admin action-desactive" id="{{ $user->id }}" state="0" data_toggle="tooltip" data-placement="bottom" title="Desactivar"><i class="fa fa-times" aria-hidden="true"></i></div>
                                                        ...
                                                        <div class="actions-btn delete-admin action-delete" id="{{ $user->id }}" data_toggle="tooltip" data-placement="bottom" title="Eliminar"><i class="fa fa-trash-o" aria-hidden="true"></i></div>
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
